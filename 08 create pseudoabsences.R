...
# Load data ---------------------------------------------------------------
pnts <- read_excel('./proj_data/bd_bjm/bd_presencias.xlsx')
pnts

# To read the climate -----------------------------------------------------
fles <- c(dir_ls('./proj_data/tif/terraclimate/extent/individual/bios', regexp = '.tif$'),
          dir_ls('./proj_data/tif/terraclimate/extent/individual/bios/v2', regexp = '.tif$')) %>% as.character() %>% mixedsort()
...
write.xlsx(vars, '/proj_data/tble/points/vars_species.xlsx')
...
fnal <- bind_rows(fnal)
write.xlsx(fnal, '/proj_data/tble/points/occr_back_species.xlsx')
