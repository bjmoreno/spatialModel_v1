...
# Load data ---------------------------------------------------------------
fles <- dir_ls('./proj_data/tif/terraclimate/extent/individual') %>% as.character(.)

# Grepping (filtering)
prec <- grep('prec', fles, value = TRUE) %>% grep('gwr_raw_150m_v2.tif$', ., value = T) %>% mixedsort()
etps <- grep('etp', fles, value = TRUE) %>% mixedsort()
tmax <- grep('tmax', fles, value = TRUE) %>% grep('gwr_raw_150m_v2.tif$', ., value = T) %>% mixedsort()
tmin <- grep('tmin', fles, value = TRUE) %>% grep('gwr_raw_150m_v2.tif$', ., value = T) %>% mixedsort()

# Regions 
rgns <- sf::st_read('./proj_data/gpkg/base/study_zone_areasp.gpkg')
...
makeBIOS <- function(yr){
  ...
  purrr::map(.x = 1:nrow(rgns), .f = function(z){
    ...
    ou  <- glue('./proj_data/tif/terraclimate/extent/individual/zones/v2/bios_{rgn$gid}_{yr}.tif')
...
rstr <- terra::rast('./proj_data/tif/terraclimate/extent/individual/zones/v2/bios_1_2016.tif')

# Mosaicking each bioclimatic var 
vars <- glue('bios_{1:19}') %>% as.character()
fles <- dir_ls('./proj_data/tif/terraclimate/extent/individual/zones/v2', regexp = '.tif$') %>% as.character() %>% grep(paste0(vars, collapse = '|'), ., value = T)
# i <- 6; j <- 4
yrs <- 2016:2019
...
purrr::map(.x = 1:length(yrs), .f = function(i){ # ciclo por variable, (bio 21 a bio 29)
  ...
  purrr::map(.x = 2:19, .f = function(j){ # ciclo por ano, (2016 a 2019)
    ...
    out <- glue('./proj_data/tif/terraclimate/extent/individual/bios/v2/{vars[j]}_{yrs[i]}.tif')
