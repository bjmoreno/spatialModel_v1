...
fles <- c(dir_ls('./proj_data/tif/terraclimate/extent/individual/bios', regexp = '.tif$'),
          dir_ls('./proj_data/tif/terraclimate/extent/individual/bios/v2', regexp = '.tif$'))
...
data_edit(lbls)
lbls <- read_csv('./proj_data/tble/labels_species.csv')[,-1]
...
vles <- bind_rows(vles)
write.xlsx(vles, './proj_data/tble/values_points.xlsx')
